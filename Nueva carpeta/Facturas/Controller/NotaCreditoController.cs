...
namespace Facturas.Controller
{
    public class NotaCreditoController
    {
        ...
        private string oCodTipNC;
        private string oDscTipNC;
        #endregion
        ...
        {
            Conexion.EjecutarQuery(string.Format("EXEC [dbo].[Actualizar_Rpta_Cdr_NC]@Docentry = {0}, @U_ResponseCode = '{1}', @U_Description = '{2}'", oDocEntry, respuestacdr[0], respuestacdr[1]));
        }

        public NotaCreditoController(SAPbouiCOM.Form oOrderForm)
        {
            try
            {
                Datos(oOrderForm);
